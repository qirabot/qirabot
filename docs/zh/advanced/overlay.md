...
- **截图排除**(macOS `NSWindowSharingNone`、Windows
  `WDA_EXCLUDEFROMCAPTURE`):不会出现在 bot 自己的截图里,也就不会干扰模型。
- **点击穿透**,不会挡住要点给下层应用的点击。
...
当任务驱动的是真实键鼠(桌面后端:`Window`、pyautogui)时,任务期间
屏幕四边会亮起缓慢呼吸的琥珀色光晕,和屏幕共享边框是同一套视觉语言:
*机器正被接管,请勿操作*。远程协议目标(浏览器、Android、iOS)不会点亮
它,那些场景下你的键鼠依然是你的。

光晕与窗口一样被截图排除,且规则更严格:在不支持排除的 Windows 版本上
它干脆不显示,因为四边发光的截图会干扰模型识别。
...
ESC 中止")。长按 ESC 约一秒即可中止任务:

- bot 会在下一个步骤边界停下。一步可能需要几秒(在途的模型调用要先
  返回),但期间不会再注入任何输入。
- bot 按住的所有按键和鼠标键全部松开。
- `bot.ai()` 抛出 `user_abort` 错误,任务被记录为已取消
  (cancelled)而非失败,主动中止不会计入你的失败率统计。

短促的 ESC 点击不会触发,无论是你按的还是 bot 注入的。

中止具**粘性**:同一 bot 实例后续所有 `bot.ai()` 都会立即抛出
`user_abort`(不点光晕、不碰键鼠),脚本里的 try/except 无法把你刚收回
的机器再抢回去。要继续,必须显式调用 `bot.clear_user_abort()`。单步调用
不受影响,可用于善后清理。
...
常规用法只需一个参数,窗口由 bot 全自动驱动:标题行是指令 + 运行状态点 +
...
如果已经有自己的 `on_step` 回调,写 `on_step=ov.wrap(my_cb)`,两个都会被调用。
...
- **Windows**:用标准库 tkinter。完整的截图排除需要 Windows 10 2004+,
  ...
  在这些版本上干脆不显示)。
- **其余环境**(Linux、CI、无 GUI):悬浮窗静默不生效,不会影响任务
  本身。
