...
在新标签页打开链接时,返回值就是那个新标签页;重新赋值以继续操作
活动页面:
...
步骤回调、`custom_tools`、`exclude_tools` 的完整说明见
...
**直接输入。** `type_text` 传空 `locate` 会跳过 AI 定位,输入到当前
...
| 按住(桌面) | `duration_seconds=2`(浮点 > 0,上限 10) | 按住指定时长再释放,用于定量的游戏内移动(`w`、`shift+w`)。仅 pyautogui + Windows 窗口后端;web/移动端忽略并瞬时点按 |
...
点击在新标签页打开了链接,而新标签页没有历史)且还有其他标签页时,
就关闭当前标签页回到上一个:
...
(`extract`、`verify`、`wait_for`、`ai`)在所有框架上可用;矩阵展示的
是各底层动作在每个平台的映射方式。
...
本地运行 id(`bot.task_id`,8 位十六进制),每次
