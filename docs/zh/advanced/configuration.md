...
::: warning Vertex 上的 flex 不适合多步任务
它在 Vertex 上的代价是每请求近乎固定的排队延迟,而不是更慢的生成 ——
在 `gemini-3.5-flash` 上实测为**每次调用 +12~14 秒**(2026-08)。由于
它不随响应长度缩放,一次很短的 decide/locate 调用要全额承担。一个
3 步任务在标准档只花 8 秒模型时间,在 flex 上花了 50 秒。
